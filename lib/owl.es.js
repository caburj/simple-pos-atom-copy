...
class QWeb extends EventBus {
  ...
  _compile(name, options = {}) {
    ...
    if (!options.hasParent) {
      if (ctx.shouldDefineResult) {
        ...
        if (!ctx.rootNode) {
          throw new Error(
            `A template should have one root node (${ctx.templateName})`
          );
        }
  ...
  _compileNode(node, ctx) {
    ...
    const shouldAddNS =
      node.nodeName === 'svg' ||
      (node.nodeName === 'g' && ctx.rootNode === ctx.parentNode);
    if (shouldAddNS) {
  ...
  _compileGenericNode(node, ctx, withHandlers = true) {
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      if (name.startsWith('t-att-')) {
        ...
        let formattedValue =
          typeof v === 'string' ? ctx.formatExpression(v) : `scope.${v.id}`;
        if (attName === 'class') {
          ctx.rootContext.shouldDefineUtils = true;
    ...
    let nodeID = ctx.generateID();
    let key =
      ctx.loopNumber || ctx.hasKey0
        ? `\`\${key${ctx.loopNumber}}_${nodeID}\``
        : nodeID;
    const parts = [`key:${key}`];
...
QWeb.addDirective({
  ...
  atNodeEncounter({ node, ctx }) {
    ...
    ctx.addIf(
      typeof cond === 'string' ? ctx.formatExpression(cond) : `scope.${cond.id}`
    );
    return false;
  },
  finalize({ ctx }) {
  ...
  atNodeEncounter({ node, qweb, ctx }) {
    ...
    if (isDynamic) {
      ...
      if (!subId) {
        ...
        const subTemplateFn = qweb._compile(subTemplate, {
          hasParent: true,
          defineKey: true,
        });
    ...
    if (ctx.parentNode) {
      ctx.addLine(
        `this.constructor.subTemplates[${subIdstr}].call(this, scope, ${extra});`
      );
    } else {
...
QWeb.addDirective({
  ...
  atNodeEncounter({ node, qweb, ctx }) {
    ...
    ctx.addLine(
      `for (let ${loopVar} = 0; ${loopVar} < _length${keysID}; ${loopVar}++) {`
    );
    ctx.indent();
...
function makeHandlerCode(
  ...
  let handler = `function (e) {if (context.__owl__.status === ${
    5 /* DESTROYED */
  }){return}${modCode}${code}}`;
  if (putInCache) {
    const key = ctx.generateTemplateKey(event);
    ctx.addLine(
      `extra.handlers[${key}] = extra.handlers[${key}] || ${handler};`
    );
...
QWeb.addDirective({
  ...
  atNodeEncounter({ ctx, value, node, qweb }) {
    ...
    const valueExpr = value.match(INTERP_REGEXP)
...
QWeb.addDirective({
  ...
  atNodeEncounter({ ctx, value, node, qweb }) {
    ...
    if (
      node.tagName === 't' &&
      !node.hasAttribute('t-key') &&
      value.match(INTERP_REGEXP)
    ) {
      defineComponentKey();
    ...
    if (hasDynamicProps) {
      ...
      ctx.addLine(
        `let props${componentID} = Object.assign({}, ${dynamicProp}, {${propStr}});`
      );
    } else {
    ...
    let styleCode = '';
    if (tattStyle) {
      styleCode = `.then(()=>{if (w${componentID}.__owl__.status === ${
        5 /* DESTROYED */
      }) {return};w${componentID}.el.style=${tattStyle};});`;
    }
    ctx.addLine(
    ...
    defineComponentKey();
    const contextualValue = value.match(INTERP_REGEXP)
      ? 'false'
      : ctx.formatExpression(value);
    ctx.addLine(
    ...
    ctx.addLine(
      `w${componentID} = new W${componentID}(parent, props${componentID});`
    );
    if (transition) {
    ...
    ctx.addLine(
      `parent.__owl__.cmap[${templateKey}] = w${componentID}.__owl__.id;`
    );
    if (hasSlots) {
      ...
      if (slotNodes.length) {
        for (let i = 0, length = slotNodes.length; i < length; i++) {
          ...
          const slotFn = qweb._compile(`slot_${key}_template`, {
            elem: slotNode,
            hasParent: true,
          });
        ...
        for (let child of Object.values(clone.childNodes)) {
          hasContent =
            ...
            (child instanceof Text
              ? Boolean(child.textContent.trim().length)
              : true);
          t.appendChild(child);
        }
        if (hasContent) {
          const slotFn = qweb._compile(`slot_default_template`, {
            elem: t,
            hasParent: true,
          });
    ...
    for (let i = patchLen - 1; i >= 0; i--) {
      ...
      if (fiber.target && i === 0) {
        ...
        if (fiber.position === 'self') {
          ...
          const selfVnodeData = fiber.vnode.data
            ? { key: fiber.vnode.data.key }
            : {};
          ...
          target =
            component.__owl__.vnode || document.createElement(fiber.vnode.sel);
        }
        ...
        if (fiber.shouldPatch && vnode) {
          ...
          component.__patch(
            document.createElement(fiber.vnode.sel),
            fiber.vnode
          );
          component.__owl__.pvnode.elm = component.__owl__.vnode.elm;
        }
  ...
  if (propsDef instanceof Array) {
    ...
    for (let i = 0, l = propsDef.length; i < l; i++) {
      ...
      if (!(propName in props)) {
        throw new Error(
          `Missing props '${propsDef[i]}' (component '${Widget.name}')`
        );
      }
    }
    for (let key in props) {
      if (!propsDef.includes(key) && !propsDef.includes(key + '?')) {
        throw new Error(
          `Unknown prop '${key}' given to component '${Widget.name}'`
        );
      }
    }
  } else if (propsDef) {
    ...
    for (let propName in propsDef) {
      if (props[propName] === undefined) {
        if (propsDef[propName] && !propsDef[propName].optional) {
          throw new Error(
            `Missing props '${propName}' (component '${Widget.name}')`
          );
        } else {
          continue;
        }
      ...
      if (!isValid) {
        throw new Error(
          `Invalid Prop '${propName}' in component '${Widget.name}'`
        );
      }
    }
    for (let propName in props) {
      if (!(propName in propsDef)) {
        throw new Error(
          `Unknown prop '${propName}' given to component '${Widget.name}'`
        );
      }
    }
  }
...
class Component {
  ...
  async mount(target, options = {}) {
    if (
      !(target instanceof HTMLElement || target instanceof DocumentFragment)
    ) {
  ...
  __render(fiber) {
    ...
    if (__owl__.hasRendered) {
      if (__owl__.willRerenderCB) {
        __owl__.willRerenderCB();
      }
      this.willRerender();
    } else {
      __owl__.hasRendered = true;
    }
    let error;
    try {
      ...
      for (let childKey in __owl__.children) {
        ...
        if (
          childOwl.status !== 3 /* MOUNTED */ &&
          childOwl.parentLastFiberId < fiber.id
        ) {
  ...
  return function (cb) {
    ...
    if (component.__owl__[method]) {
      ...
      component.__owl__[method] = function (...args) {
        return Promise.all([
          current.call(component, ...args),
          cb.call(component, ...args),
        ]);
      };
    } else {
      component.__owl__[method] = cb;
...
const onWillRerender = makeLifecycleHook('willRerenderCB', true);
...
function useStore(selector, options = {}) {
  ...
  if (!(store instanceof Store$1)) {
    throw new Error(
      `No store found when connecting '${component.constructor.name}'`
    );
  }
...
class Portal extends Component {
  ...
  __checkVNodeStructure(vnode) {
    ...
    if (countRealNodes !== 1) {
      throw new Error(
        `Portal must have exactly one non-text child (has ${countRealNodes})`
      );
    }
  }
  /**
  ...
  __patch(target, vnode) {
    ...
    const portalPatch = this.portal
      ? this.portal
      : document.createElement(vnode.children[0].sel);
...
class RouteComponent extends Component {
  get routeComponent() {
    return (
      this.env.router.currentRoute && this.env.router.currentRoute.component
    );
  }
}
...
class Router {
  constructor(env, routes, options = { mode: 'history' }) {
    ...
    for (let partialRoute of routes) {
      ...
      if (partialRoute.component) {
        QWeb.registerComponent(
          '__component__' + partialRoute.name,
      ...
      partialRoute.params = partialRoute.path
        ? findParams(partialRoute.path)
        : [];
  ...
  async _navigate(path, ev) {
    ...
    const didChange =
      this.currentRouteName !== initialName ||
      !shallowEqual(this.currentParams, initialParams);
    if (didChange) {
...
__info__.date = '2021-11-03T12:44:47.214Z';
